...
int main(int argc, char **argv)
{
    ...
    for(int ni=1; ni<nImages; ni++)
    {
        ...
        imRGB2 = cv::imread(string(argv[3])+"/"+vstrImageFilenamesRGB[ni-1],CV_LOAD_IMAGE_UNCHANGED);
        imD2 = cv::imread(string(argv[3])+"/"+vstrImageFilenamesD[ni-1],CV_LOAD_IMAGE_UNCHANGED);
        ...
        if (argc == 6 || argc == 7){
            mask = yolo->Segmentation(imRGB2,imRGB);
            // cv::Mat maskYOLOdil = mask.clone();
            // cv::dilate(mask,maskYOLOdil, kernel);
